...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_rate=0., layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # group=dim，深度卷积，每个通道都用一个卷积核处理。
        self.norm = LayerNorm(dim, eps=1e-6, data_format="channels_last")
        self.pwconv1 = nn.Linear(dim, 4 * dim)  # 1×1卷积（pointwise conv 点卷积） 作用相当于Linear
        self.act = nn.GELU()  # 激活函数
        self.pwconv2 = nn.Linear(4 * dim, dim)  # pointwise conv
    ...
    def forward(self, x):
        shortcut = x
        x = self.dwconv(x)
        x = x.permute(0, 2, 3, 1)  # [B, C, H, W] -> [B, H, W, C] 将通道数移到最后一个位置，之后在进行LN
        x = self.norm(x)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.pwconv2(x)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 1, 2)  # [B, H, W, C] -> [B, C, H, W] 将通道数的位置还原回来
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths: list = None, dims: list = None,
        ...
        stem = nn.Sequential(nn.Conv2d(in_chans, dims[0], kernel_size=4, stride=4),
                             LayerNorm(dims[0], eps=1e-6, data_format='channels_first'))  # 最开始的输入图像之后部分
