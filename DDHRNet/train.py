...
from reader import SegDataset, SarSegDataset
...
def train(cfg):
    ...
    # print('The flops of model is {}'.format(slim.analysis.flops(train_prog, inputs=image)))
    # print('The param of model is {}'.format(slim.analysis.model_size(train_prog)))
    # print(paddle.flops(train_prog, [1,3,256,256]))
    for epoch in range(begin_epoch, cfg.SOLVER.NUM_EPOCHS + 1):
            ...
            try:
                if args.debug:
                    ...
                    loss, lr, pred, grts, masks = exe.run(
                        ...
                        return_numpy=True)
                    # print('one image time is:{}s'.format(time()-one_image_time))
        ...
        if (epoch % cfg.TRAIN.SNAPSHOT_EPOCH == 0
            ...
            if args.do_eval:
                ...
                if args.use_vdl:
                    log_writer.add_scalar('Evaluate/mean_iou', mean_iou*100, step)
                    log_writer.add_scalar('Evaluate/mean_acc', mean_acc*100, step)

                if mean_iou > best_mIoU:
                    best_mIoU = mean_iou
                    update_best_model(ckpt_dir)
                    print_info("Save best model {} to {}, mIoU = {:.2f}".format(
                        ckpt_dir,
    ...
    print_info("The best mIoU is: ", best_mIoU)
