...
class SegDataset(object):
    def __init__(self,
        ...
        with codecs.open(file_list, 'r', 'utf-8') as flist:
            ...
            self.all_lines = copy.deepcopy(self.lines)  # 防止修改原数据
    ...
    def load_image(self, line, src_dir, mode=ModelPhase.TRAIN):
        ...
        parts = line.strip().split(cfg.DATASET.SEPARATOR)  # cfg - config   grt -  ground truth
        if len(parts) != 2:
    ...
    def process_image(self, line, data_dir, mode):
        ...
        if mode == ModelPhase.TRAIN:
            ...
            if cfg.AUG.MIRROR:  # 图片对称处理-水平翻转
                if np.random.randint(0, 2) == 1:
                    img = img[:, ::-1, :]
                    grt = grt[:, ::-1]

            img, grt = aug.rand_crop(img, grt, mode=mode)  # 随机裁剪
...
class SarSegDataset(object):
    ...
    def batch(self, reader, batch_size, is_test=False, drop_last=False):
        def batch_reader(is_test=False, drop_last=drop_last):
            if is_test:
                imgs, sar_imgs, grts, img_names, valid_shapes, org_shapes = [], [], [], [], [], []
                for img, sar_img, grt, img_name, valid_shape, org_shape in reader():
                    imgs.append(img)
                    ...
                    org_shapes.append(org_shape)
                    if len(imgs) == batch_size:
                        yield np.array(imgs), np.array(sar_imgs), np.array(
                            grts), img_names, np.array(valid_shapes), np.array(
                            org_shapes)
                        imgs, sar_imgs, grts, img_names, valid_shapes, org_shapes = [], [], [], [], [], []

                if not drop_last and len(imgs) > 0:
                    yield np.array(imgs), np.array(sar_imgs), np.array(grts), img_names, np.array(
                        valid_shapes), np.array(org_shapes)
            else:
                imgs, sar_imgs, labs, ignore = [], [], [], []
                bs = 0
                for img, sar_img, lab, ig in reader():
                    imgs.append(img)
                    sar_imgs.append(sar_img)
                    labs.append(lab)
                    ignore.append(ig)
                    bs += 1
                    if bs == batch_size:
                        yield np.array(imgs), np.array(sar_imgs), np.array(labs), np.array(ignore)
                        bs = 0
                        imgs, sar_imgs, labs, ignore = [], [], [], []
    ...
    def load_image(self, line, src_dir, mode=ModelPhase.TRAIN):
        ...
        # parts是一行中按分隔符分开的几个文件名
        img_name, sar_name, grt_name = parts[0], parts[1], parts[2]

        img_path = os.path.join(src_dir, img_name)
        sar_img_path = os.path.join(src_dir, sar_name)

        img = cv2_imread(img_path, cv2_imread_flag)
        # 下面如果不对要改成PIL读取
        # 现在读进来是三通道的图像，后面网络中的大小设置可能要改
        sar_img = cv2_imread(sar_img_path)
        ...
        return img, sar_img, grt, img_name, grt_name
    ...
    def process_image(self, line, data_dir, mode):
        """ process_image """
        # line：‘'JPEGImages/P0147.jpg Annotations/P0147.png'’
        img, sar_img, grt, img_name, grt_name = self.load_image(
            line, data_dir, mode=mode)
        if mode == ModelPhase.TRAIN:
            img, grt = aug.resize(img, grt, mode)
            sar_img, _ = aug.resize(sar_img, grt, mode)
            ...
            img, grt = aug.rand_crop(img, grt, mode=mode)
            sar_img, _ = aug.rand_crop(sar_img, grt, mode=mode)
        elif ModelPhase.is_eval(mode):
            img, grt = aug.resize(img, grt, mode=mode)
            img, grt = aug.rand_crop(img, grt, mode=mode)

            sar_img, _ = aug.resize(sar_img, grt, mode=mode)
            sar_img, _ = aug.rand_crop(sar_img, grt, mode=mode)
        ...
        if ModelPhase.is_train(mode):
            return (img, sar_img, grt, ignore)
        elif ModelPhase.is_eval(mode):
            return (img, sar_img, grt, ignore)
        elif ModelPhase.is_visual(mode):
            return (img, sar_img, grt, img_name, valid_shape, org_shape)
